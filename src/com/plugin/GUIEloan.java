              ...
              if ((StringUtils.isNotBlank(ss)) && (!"null".equals(ss))) {
                my_amount = Integer.valueOf(Integer.parseInt(ss));
						...
						//if (my_amount != null) {
							...
							String getLeftAmount = GUIEloan.this.amount.getText();
							...
							GUIEloan.this.writeUserConfig(getName, getPwd, getPayPassword, getPayAmount, getLeftAmount, getMinRate, getRefreshTime, getRefreshTimeOut, getBuyTimeOut, getMultiThread);
							...
							transferWorker.setLeftAmount(StringUtils.isNotBlank(getLeftAmount) ? Integer.parseInt(getLeftAmount) : 0);
...
/*						} else {
		...
		this.bu2 = new JButton("读取配置");
		this.bu2.setBounds(220, 300, 100, 20);

		this.bu2.addActionListener(new java.awt.event.ActionListener()
		{
			public void actionPerformed(java.awt.event.ActionEvent e)
			{
				getUserConfig(GUIEloan.this.username.getText());
			}
		});
	...
	private void writeUserConfig(String account, String password, String payPassword, String balance, String leftAmount, String minRate, String refreshTime, String refreshTimeOut, String buyTimeOut, boolean multiThread)
	{
		if(!getUserConfig(account)){
			...
			jsonObject.element("leftAmount", leftAmount);
			...
			try {
				BufferedWriter out = new BufferedWriter(new java.io.FileWriter(filePath, true));
	...
	private void getUserConfig()
	{
		try {
			...
			String leftAmount = jsonObject.getString("leftAmount");
			...
			this.amount.setText(leftAmount);
	...
	private boolean getUserConfig(String user)
	{
		try {
			String filePath = System.getProperty("user.dir");
			filePath = filePath + java.io.File.separator + "user_config";
